...
function pickingNumbers(a) {
    ...
    let original_array = a;
    let answer = 0;
    original_array.sort();

    //console.log(original_array);

    let blackboard = []; //used to swap

    while(original_array.length > 1) {
      blackboard.length = 0; //empty array
      blackboard.push(original_array.shift());

      while ( (original_array[0] == blackboard[0]) || (original_array[0]-blackboard[0] == 1) ) {
            blackboard.push(original_array.shift());
      }

      if (blackboard.length > answer) {
            answer = blackboard.length;
      }
    }

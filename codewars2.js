/**
Given an array of digitals numbers, return a new array of length number 
containing the last even numbers from the original array (in the same order). 
The original array will be not empty and will contain at least "number" even numbers.

For example:

([1, 2, 3, 4, 5, 6, 7, 8, 9], 3) => [4, 6, 8]
([-22, 5, 3, 11, 26, -6, -7, -8, -9, -8, 26], 2) => [-8, 26]
([6, -25, 3, 7, 5, 5, 7, -3, 23], 1) => [6]
*/

function evenNumbers2(array, number) {
  console.log('array = ' + array);
  console.log('number = ' + number);
  // good luck
  let newArr = [];
  let popped = 0;

  while( newArr.length < number ) {
     popped = array.pop();
     if (popped % 2 == 0) {
         newArr.unshift(popped);
     }
   }
  console.log(newArr);
  return newArr;
}

/**
more efficient
*/
function evenNumbers(array, number) {
  console.log('array = ' + array);
  console.log('number = ' + number);
  // good luck
  let newArr = [];
  let popped = 0;

  for (let i=array.length-1; i>=0; i--) {
      if ( newArr.length < number ) {
         
         if (array[i] % 2 == 0) {
             newArr.unshift(array[i]);
         }
      } else
      break;
      
  }
  console.log(newArr);
  return newArr;
}

function evenNumbers3(array, number) {
  console.log('array = ' + array);
  console.log('number = ' + number);
  // good luck

  let newArr = array.filter(function(element){
      if (element % 2 == 0) return element;
  });

  console.log(newArr.slice(-number));
  return newArr.slice(-number);

}

/*
shortcut of above
*/
function evenNumbers4(array, number) {
  return array.filter(function(element){
      if (element % 2 == 0) return element;
  }).slice(-number);
}

evenNumbers3([1, 2, 3, 4, 5, 6, 7, 8, 9], 3);
evenNumbers3([-22, 5, 3, 11, 26, -6, -7, -8, -9, -8, 26], 2);
evenNumbers3([6, -25, 3, 7, 5, 5, 7, -3, 23], 1);
//testing
/*
Test.describe("Basic tests",() => {
  Test.assertDeepEquals(evenNumbers([1, 2, 3, 4, 5, 6, 7, 8, 9], 3), [4, 6, 8]);
  Test.assertDeepEquals(evenNumbers([-22, 5, 3, 11, 26, -6, -7, -8, -9, -8, 26], 2), [-8, 26]);
  Test.assertDeepEquals(evenNumbers([6, -25, 3, 7, 5, 5, 7, -3, 23], 1), [6]);
});*/